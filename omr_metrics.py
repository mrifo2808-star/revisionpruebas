...
def evaluar_respuestas(pred: list, real: list) -> dict:
    n = len(real)
    aciertos = 0
...
def correr_metodo_omr(app_module, datos_bytes: bytes, solo_respuestas: bool, n: int) -> dict:
    ...
    return {"respuestas": respuestas, "tiempo_s": elapsed, "llamadas_api": 0,
...
def correr_metodo_ia(app_module, cliente, datos_bytes: bytes, solo_respuestas: bool, n: int) -> dict:
    ...
    return {"respuestas": res["respuestas"], "tiempo_s": elapsed,
            "llamadas_api": res.get("intentos", 1), "pct_resuelto_sin_ia": 0.0}
    ...
    for caso in casos:
        ...
        for metodo in metodos:
            try:
                ...
                continue
            ev = evaluar_respuestas(salida["respuestas"], real)
            ...
            print(f"[{metodo}] {caso['imagen']}: accuracy={ev['accuracy']*100:.1f}% "
                  ...
                  f"llamadas_api={salida['llamadas_api']}")
